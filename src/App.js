...
function App() {
  const [widgets, setWidgets] = useState([
    ...
    { id: new Date().getTime() + 3, component: <PhotoWidget imageUrl="https://example.com/photo1.jpg" caption="Beautiful sunset" />, area: 'right-widget', name: 'Photo Gallery' },
  ]);
            ...
            {widgets.length > 0 &&
              widgets.map((widget, index) => {
                if (widget.area === 'left-widget') {
                  return (
                    <div key={widget.id} style={{ padding: 10 }} className="widget-container">
                      <div style={{ marginBottom: 10 }}>
                        <select
                          value={widget.area}
                          onChange={(e) => {
